...
Description of the illustration json_table_on_empty_clause.gif
...
(*JSON_value_return_type::=* and *JSON_value_on_error_clause::=*-part of `JSON_VALUE`, *JSON_basic_path_expression*: See *Oracle Database JSON Developer’s Guide*)
## *JSON_nested_path*::=
Description of the illustration json_nested_path.gif
Description of the illustration json_nested_path.eps
(*JSON_basic_path_expression*: See *Oracle Database JSON Developer’s Guide*, *JSON_columns_clause::=*)
## *ordinality_column*::=
...
## *nested_clause*::=
Description of the illustration nested_clause.gif
...
**See Also:**   in *Oracle Database Globalization Support Guide* for the collation derivation rules, which define the collation assigned to each character data type column in the table generated by `JSON_TABLE`
## *expr*
...
**
****
  - If the input is not well-formed JSONtext, no more rows will be returned as soon as the error is detected. Note that since JSON_TABLE supports streaming evaluation, rows may be returned prior to encountering the portion of the input with the error.
