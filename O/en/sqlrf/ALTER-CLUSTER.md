...
**See Also:**   CREATE CLUSTER for information on creating a cluster, DROP CLUSTER and DROP TABLE for information on removing tables from a cluster, and CREATE TABLE … *physical_properties* for information on adding a table to a cluster
## Prerequisites
The cluster must be in your own schema or you must have the `ALTER` `ANY` `CLUSTER` system privilege.
## Syntax
## *alter_cluster*::=
Description of the illustration alter_cluster.eps
...
## Semantics
## IF EXISTS
Specify `IF EXISTS` to alter an existing cluster.
...
```
  ...
  **See Also:**
  ...
  - *[storage_clause](physical_attributes_clause.html#GUID-A15063A9-3237-43D3-B0AE-D01F6E80B393\_\_I1026834)* for a full description of that clause
  </div>
```
...
**See Also:**   CREATE CLUSTER for a description of the `SIZE` parameter and “Modifying a Cluster: Example”
## MODIFY PARTITION
...
**See Also:**   *parallel_clause* in the documentation on `CREATE` `TABLE` for complete information on this clause
## Examples
