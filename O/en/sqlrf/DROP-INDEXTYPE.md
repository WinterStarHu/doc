...
**See Also:**   CREATE INDEXTYPE for more information on indextypes
## Prerequisites
The indextype must be in your own schema or you must have the `DROP` `ANY` `INDEXTYPE` system privilege.
## Syntax
## *drop_indextype*::=
Description of the illustration drop_indextype.eps
## Semantics
## IF EXISTS
Specify `IF EXISTS` to drop an existing indextype.
